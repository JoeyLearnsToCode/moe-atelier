...
function App() {
  ...
  const fetchModels = async () => {
    ...
    try {
      ...
      const versionFallback =
        apiFormat === 'openai' || apiFormat === 'openai-image' || apiFormat === 'openai-hybrid' ? 'v1' : apiFormat === 'vertex' ? 'v1beta1' : 'v1beta';
      ...
      if (apiFormat === 'openai' || apiFormat === 'openai-image' || apiFormat === 'openai-hybrid') {
  ...
  const handleCreateTaskFromPrompt = (prompt: string) => {
    ...
    saveTaskState(storageKey, {
      ...
      concurrency: 2,
      enableSound: true,
      retryInterval: (config.defaultRetryInterval ?? 1) * 1000,
      retryLimit: config.defaultRetryLimit ?? -1,
      generationCount: config.defaultGenerationCount ?? 1,
      apiProfileId: config.defaultApiProfileId || config.activeApiProfileId || 'default',
      results: [],
      uploads: [],
  ...
  const handleDownloadAll = useCallback(async () => {
    ...
    try {
      const lastDownloadTime = config.skipDownloadedInDownloadAll !== false
        ...
        : undefined;
      const pack = await buildBatchDownloadZip(collectedItems, {
